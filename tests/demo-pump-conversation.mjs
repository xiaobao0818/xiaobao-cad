...
try {
  ...
  const result = await page.evaluate(async (req) => {
    ...
    const task = taskById('conversation3d'); // 裸对话需求任务（与 mock 自然语言识别一致）
    // 场景清空 + 3D 工作区
    ...
    const evalNow = () => evaluate(task, { bodies: model.serialize().bodies });
    score = evalNow().score;
    ...
    const bodies = model.serialize().bodies;
