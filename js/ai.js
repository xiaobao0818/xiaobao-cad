...
const SYSTEM_PROMPT = [
  ...
  '【首轮纪律】用户提出建模/绘图需求后，第一轮回复就必须调用工具实际创建实体（create_primitive_3d / draw_entities 等）；严禁只输出方案、计划或说明文字而不调用工具。若上一步失败，再调用工具修正，而不是继续用文字解释。若用户给出泵的工况（流量/扬程/转速），第一轮必须先调用 pump_sizing 算出设计尺寸，再按尺寸创建实体。',
